...
pub struct BitVec<const N: usize>(BigInt, Arc<BigInt>, bool);
...
impl<const N: usize> BitVec<N> {
    fn mask_value() -> Arc<BigInt> {
        Arc::new((BigInt::from(1) << N as u32) - BigInt::from(1))
    }

    fn mask(self) -> Self {
        Self(self.0 & &*self.1, self.1, false)
    }
    ...
    pub fn signed_borrow(&self, rhs: &Self) -> bool {
        ...
        let min = -(BigInt::from(1) << (N - 1) as u32);
    ...
    pub fn carry(&self, rhs: &Self) -> bool {
        if self.is_signed() || rhs.is_signed() {
            ...
            let max = (BigInt::from(1) << N as u32) - BigInt::from(1);
    ...
    pub fn signed_carry(&self, rhs: &Self) -> bool {
        ...
        let max = (BigInt::from(1) << (N - 1) as u32) - BigInt::from(1);
    ...
    pub fn max_value(signed: bool) -> Self {
        if signed {
    ...
    pub fn min_value(signed: bool) -> Self {
        if signed {
...
impl<const N: usize> From<BitVec<N>> for crate::BitVec {
    fn from(bv: BitVec<N>) -> Self {
        let val = crate::BitVec::from_bigint_with(bv.0, bv.1);
        if bv.2 {
            val.signed()
        } else {
            val
...
impl<const N: usize> BitVec<N> {
    pub fn gcd(&self, rhs: &Self) -> Self {
        if self.is_signed() || rhs.is_signed() {
            ...
            match (lmsb, rmsb) {
                (false, false) => BitVec::from(BigInt::from(self.0.gcd_ref(&rhs.0))),
                (true, false) => BitVec::from((-(-self).0).gcd(&rhs.0)),
                (false, true) => BitVec::from(BigInt::from(self.0.gcd_ref(&-(-rhs).0))),
                (true, true) => BitVec::from((-(-self).0).gcd(&-(-rhs).0)),
            }
        } else {
            BitVec::from(BigInt::from(self.0.gcd_ref(&rhs.0)))
        }
    }
...
impl<const N: usize> Shl for BitVec<N> {
    type Output = Self;

    fn shl(self, rhs: Self) -> Self::Output {
        if matches!(rhs.0.to_usize(), Some(v) if v > self.bits()) {
            BitVec::zero()
        } else {
...
impl<'a, const N: usize> Shl for &'a BitVec<N> {
    type Output = BitVec<N>;

    fn shl(self, rhs: Self) -> Self::Output {
        if matches!(rhs.0.to_usize(), Some(v)  if v >= N) {
            BitVec::zero()
        } else {
            if let Some(rhs) = rhs.0.to_u32() {
                BitVec::from(BigInt::from(&self.0 << rhs))
...
impl<const N: usize> Shr<u32> for BitVec<N> {
    ...
    fn shr(self, rhs: u32) -> Self::Output {
        ...
        } else if self.is_negative() { // perform ASR
...
impl<const N: usize> Shr for BitVec<N> {
    type Output = Self;

    fn shr(self, rhs: Self) -> Self::Output {
        if matches!(rhs.0.to_usize(), Some(v) if v as usize >= N) {
            if self.is_signed() {
                -BitVec::one()
            } else {
                BitVec::zero()
...
impl<'a, const N: usize> Shr for &'a BitVec<N> {
    type Output = BitVec<N>;
...
#[derive(Debug, Error)]
pub enum ParseBitVecError {
    #[error("parsed value out of range for bit-vector of size {0}")]
    OutOfRange(usize),
    #[error(transparent)]
    Parse(#[from] rug::integer::ParseIntegerError),
}

impl<const N: usize> Num for BitVec<N> {
    type FromStrRadixErr = ParseBitVecError;

    fn from_str_radix(value: &str, radix: u32) -> Result<Self, Self::FromStrRadixErr> {
        let parsed = BigInt::from_str_radix(value, radix as i32)?;
        if parsed < 0 {
            if parsed < -(BigInt::from(1) << (N - 1) as u32) {
                Err(ParseBitVecError::OutOfRange(N))
            } else {
                Ok(BitVec::from(parsed))
            }
        } else {
            if parsed > (BigInt::from(1) << N as u32) - 1 {
                Err(ParseBitVecError::OutOfRange(N))
            } else {
                Ok(BitVec::from(parsed))
            }
...
macro_rules! impl_from_for {
    ($t:ident) => {
        impl<const N: usize> From<$t> for BitVec<N> {
            fn from(t: $t) -> Self {
                BitVec::from(BigInt::from(t))
            }
        }
    };
}
...
mod test {
    ...
    fn test_signed_shift_right() {
        ...
        let v4 = BitVec::<16>::from_u16(0x05deu16).unwrap();
        assert_eq!(v4.signed() >> 1u32, BitVec::<16>::from_u16(0x2efu16).unwrap());
    }
