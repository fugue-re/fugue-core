    ...
    pub fn value_with<'b, 'c, 'z>(
        ...
        Ok(match self {
            Self::TokenField {
                ...
                let offset = (*byte_start + unsafe { walker.offset(None) }) as u32 * 8;
                let end_offset = size as u32 * 8;

                (
                    value,
                    Some(
                        offset + end_offset - (*bit_end as u32 + 1)
                            ..offset + end_offset - *bit_start as u32,
                    ),
                )
            }
            Self::ContextField {
            ...
            Self::Operand {
                ...
                let (value, bits) =
                    unsafe { walker.resolve_with_bits(pexp, ctor, *index, symbols)? };
                (
                    value,
                    bits.map(|r| {
    ...
    pub fn value<'b, 'c, 'z>(
        ...
        Ok(match self {
            ...
            Self::Operand {
                ...
                unsafe { walker.resolve_with(pexp, ctor, *index, symbols)? }
            }
            ...
            Self::Next2Instruction => walker
                ...
                .map(|a| a.offset() as i64)
                .unwrap_or(0),
            Self::Plus(ref lhs, ref rhs) => lhs
                .value(walker, symbols)?
                .wrapping_add(rhs.value(walker, symbols)?),
            Self::Sub(ref lhs, ref rhs) => lhs
                .value(walker, symbols)?
                .wrapping_sub(rhs.value(walker, symbols)?),
            Self::Mult(ref lhs, ref rhs) => lhs
                .value(walker, symbols)?
