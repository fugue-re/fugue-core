    ...
    pub fn format_mnemonic<'a, 'b>(&self, fmt: &mut fmt::Formatter, walker: &mut ParserWalker<'a, 'b>, symbols: &'a SymbolTable) -> Result<(), fmt::Error> {
        if let Some(index) = self.flow_through_index {
            match symbols.symbol(self.operands[index]).expect("symbol").defining_symbol(symbols).expect("state is consistent") {
                ...
                Some(_) => {
                    walker.push_operand(index).expect("push operand");
                    walker.constructor().expect("state is consistent")
                        .expect("constructor")
            ...
            match symbols.symbol(self.operands[index]).expect("symbol").defining_symbol(symbols).expect("state is consistent") {
                ...
                Some(_) => {
                    ...
                    walker.constructor()
                        ...
                        .expect("constructor").format_body(fmt, walker, symbols).expect("format body");
                    walker.pop_operand().expect("pop operand");
                    return Ok(());
                }
            }
    ...
    pub fn from_xml(input: xml::Node) -> Result<Self, DeserialiseError> {
        ...
        for input in input.children().filter(xml::Node::is_element) {
            match input.tag_name().name() {
                ...
                _ => {
                    let cur = ConstructTpl::from_xml(input)?;
        ...
        Ok(Self {
            ...
            line_number: input.attribute_int("line")?,
            operands,
            print_pieces,
            context,
            template,
