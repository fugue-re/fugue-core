...
pub use fugue_arch::Architecture;
pub use fugue_bytes::endian::Endian;

pub(crate) fn from_reader(reader: schema::architecture::Reader) -> Result<Architecture, Error> {
    ...
    Ok(Architecture::new(name, endian, bits, variant))
}

pub(crate) fn to_builder(arch: &Architecture, builder: &mut schema::architecture::Builder) -> Result<(), Error> {
    builder.set_name(arch.name());
